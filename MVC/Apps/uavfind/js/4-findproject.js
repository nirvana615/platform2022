...
//关联模型
function LinkModel(uavfindprojectid) {
    var addmodellayerindex = layer.open({
        ...
        , success: function (layero) {
            ...
            var usedmodelids = [];
            for (var i in findprojectlist) {
                if (findprojectlist[i].id == uavfindprojectid) {
                    for (var j in findprojectlist[i].children) {
            ...
            var uavfindprojectaddmodels = [];//选中模型
                    ...
                    if (obj.checked) {
                        ...
                        if (obj.data.type == "projectnode") {
                            ...
                            for (var i in obj.data.children) {
                                ...
                                uavfindprojectaddmodels.push(model);
                            }
                        }
                        else {
                            //模型节点
                        ...
                        if (obj.data.type == "projectnode") {
                            //项目节点
                            var newuavfindprojectaddmodels = [];

                            for (var i in uavfindprojectaddmodels) {
                                if (uavfindprojectaddmodels[i].projectid != obj.data.id) {
                                    newuavfindprojectaddmodels.push(uavfindprojectaddmodels[i]);
                                }
                            }

                            uavfindprojectaddmodels = newuavfindprojectaddmodels;
                        }
                        else {
                            //模型节点
                            var newuavfindprojectaddmodels = [];
                            ...
                            for (var i in uavfindprojectaddmodels) {
                                if (uavfindprojectaddmodels[i].modelid != obj.data.id) {
                                    newuavfindprojectaddmodels.push(uavfindprojectaddmodels[i]);
                                }
                            }
            ...
            form.on('submit(addmodelusesubmit)', function (data) {
                if (uavfindprojectaddmodels.length > 0) {
                    data.field.useprojectid = uavfindprojectid;
                    data.field.cookie = document.cookie;
                    data.field.syscode = 7;
                    data.field.modelinfo = JSON.stringify(uavfindprojectaddmodels);
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if (info.code == 1) {
                ...
                for (var i in findprojectlist) {
                    if (findprojectlist[i].id == findprojectid) {
                        for (var j in findprojectlist[i].children) {
                            if (findprojectlist[i].children[j].title == "实景模型") {
                                var child = [];
