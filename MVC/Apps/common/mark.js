...
var markauthorizelayerindex = null;//标注授权弹出层
...
function exportMark(btn_id) {
    ...
    if (btn_id == "project_mark_export_id") {
        ...
        if (markProjectLayer.length == 0) {
            ...
            for (var i = 0; i < markProjectLayer[0].children.length; i++) {
                ...
                else if (markProjectLayer[0].children[i].type == "line" ) {
                    for (var j = 0; j < markProjectLayer[0].children[i].children.length; j++) {
                        if (markProjectLayer[0].children[i].children[j].checked == true) {
                ...
                else if (markProjectLayer[0].children[i].type == "polygon") {
                    for (var j = 0; j < markProjectLayer[0].children[i].children.length; j++) {
                        if (markProjectLayer[0].children[i].children[j].checked == true) {
                            polygonList.push(markProjectLayer[0].children[i].children[j]);
                        }
                    }
                }
            }
    ...
    //导出新增标注
        ...
        for (var i = 0; i < markAddLayer.length; i++) {
            ...
            else if (markAddLayer[i].type == "markadd_line" ) {
                for (var j = 0; j < markAddLayer[i].children.length; j++) {
                    if (markAddLayer[i].children[j].checked == true) {
            ...
            else if ( markAddLayer[i].type == "markadd_polygon") {
                for (var j = 0; j < markAddLayer[i].children.length; j++) {
                    if (markAddLayer[i].children[j].checked == true) {
                        polygonList.push(markAddLayer[i].children[j]);
                    }
                }
            }
        }
    ...
    var polygonKml =  '<Style id="671145735"><LineStyle><color>ff0000ff</color><width>1</width></LineStyle><PolyStyle><color>40ff0000</color></PolyStyle></Style><Folder>';
    for (var i = 0; i < polygonList.length; i++) {
        var pointListtemp = JSON.parse(polygonList[i].position);
        var polygonJson = "";
        ...
        for (var x = 0; x < pointListtemp.length; x++) {
            ...
            var L = Cesium.Math.toDegrees(cartesian3.longitude);
            var B = Cesium.Math.toDegrees(cartesian3.latitude);
            polygonJson = polygonJson + " " + L + "," + B + "," + "0 ";
        }
        polygonKml = polygonKml+'<Placemark><name>多边形</name> <description/><styleUrl>#671145735</styleUrl><Polygon><tessellate>1</tessellate><outerBoundaryIs><LinearRing><coordinates>' + polygonJson + '</coordinates></LinearRing></outerBoundaryIs></Polygon></Placemark>';
    }
    polygonKml = polygonKml + '</Folder>';
...
//标注授权
function authorizeMark(btn_id) {
    if (markauthorizelayerindex != null) {
        LayerSetTop(markauthorizelayerindex);
        return;
    }
    markauthorizelayerindex = layer.open({
        type: 1
        , title: ['标注授权', 'font-weight:bold;font-size:large;font-family:	Microsoft YaHei']
        , area: ['400px', '400px']
        , shade: 0
        , offset: 'auto'
        , closeBtn: 1
        ...
        , maxmin: true
        , resize: false
        , content:'<form class="layui-form" action="" lay-filter="markauthorizeform">    <div class="layui-form-item" style="margin-top:30px">        <div class="layui-col-md10">            <div class="grid-demo">                <label class="layui-form-label">授权系统</label>                <div class="layui-input-block">                    <select id="markauthorizeid" name="markauthorize" lay-filter="markauthorizefilter">                        <option value="">请选择</option>                        <option value="1">实景模型管理系统</option>                        <option value="2">地质灾害监测预警系统</option>                        <option value="3">航线任务规划系统</option>                        <option value="4">地质要素采集系统</option>                        <option value="5">影像对比分析系统</option>                        <option value="6">点云对比分析系统</option>                        <option value="7">无人机巡查系统</option>                    </select>                </div>            </div>        </div>    </div>    <div class="layui-form-item" style="margin-top:30px">        <div class="layui-col-md10">            <div class="grid-demo">                <label class="layui-form-label">授权用户</label>                <div class="layui-input-block">                    <select id="markauthorizeuserid" name="markauthorizeuser" lay-filter="markauthorizeuserfilter">                        <option value="">请选择</option>                    </select>                </div>            </div>        </div>    </div>    <div class="layui-form-item" style="margin-bottom:10px">        <div style="position:absolute;right:10px;bottom:10px;">            <button class="layui-btn" lay-filter="mark_authorize_confirm" id="markauthorizeconfirmid" style="width:100px">提交</button>        </div>    </div></form>'
        , zIndex: layer.zIndex
        , success: function (layero) {
            layer.setTop(layero);
            form.render();
            ...
            //授权系统ID
            var systemid;
            form.on('select(markauthorizefilter)', function (data) {
                if (data.value == "") {
                    systemid = "";
                }
                else {
            ...
            form.on('submit(markauthorizeform)', function (data) {
                ...
                for (var i = 0; i < markProjectLayer[0].children.length; i++) {               
                    for (var j = 0; j < markProjectLayer[0].children[i].children.length; j++) {
                        if (markProjectLayer[0].children[i].children[j].checked == true) {
                            var data = new Object;
                            data.id = markProjectLayer[0].children[i].children[j].id;
                            data.projetid = markProjectLayer[0].children[i].children[j].projetid;
                            authorizemarks.push(data);
                        }
                    }
                  
                }      

                authorizedata.cookie = document.cookie;
                authorizedata.authorizemarks = JSON.stringify(authorizemarks);
                $.ajax({
                    url: servicesurl + "/api/Mark/AuthorizeMark", type: "post", data: authorizedata,
                    success: function (result) {
                        var loadingmarkindex = layer.load(0, { shade: 0.2, zIndex: layer.zIndex, success: function (loadlayero) { layer.setTop(loadlayero); } });
    ...
    if (entitylabel == undefined) {
        viewer.entities.add({
            ...
            label: {
                ...
                eyeOffset: new Cesium.Cartesian3(0, 0, -10),
                disableDepthTestDistance: Number.POSITIVE_INFINITY,
                scaleByDistance: new Cesium.NearFarScalar(1000, 1, 15000, 0),
            }
        });
    }
};
