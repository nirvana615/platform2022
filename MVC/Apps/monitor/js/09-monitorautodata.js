...
var monitortreebytypedata = [];
var pushFailuretable = null;                    //推送失败数据
//自动化监测数据widget
...
function GetMonitors(projectid, index) {
                ...
                //按灾害体构建监测点树
                monitortreebytypedata = [];             //按不同分类方法组成的监测点集合
...
function EditProjectDeviceAutoData(projectid) {
    ...
    if (currentprojectmonitors.length > 0) {
        ...
        for (var i in deviceTypes) {
            if (deviceTypes[i].title != "雨量") {
                monitorDevices.push(deviceTypes[i]);
            }
        }
        if (monitorDevices.length > 0) {
            for (var i in monitorDevices) {
                for (var j in monitorDevices[i].children) {
                    if (i == 0 && j == 0) {
                        monitorArr.title = monitorDevices[i].children[j].title;
                        monitorArr.type = monitorDevices[i].children[j].type;
                        monitorArr.id = monitorDevices[i].children[j].id;
                        document.getElementById("editautodatadeviceid").innerHTML += '<option  type="' + monitorDevices[i].children[j].type + '" title="' + monitorDevices[i].children[j].title + '" value="' + monitorDevices[i].children[j].id + '" selected>' + monitorDevices[i].children[j].title + '</option>';
                    }
                    else {
                        document.getElementById("editautodatadeviceid").innerHTML += '<option type="' + monitorDevices[i].children[j].type + '" title="' + monitorDevices[i].children[j].title + '" value="' + monitorDevices[i].children[j].id + '">' + monitorDevices[i].children[j].title + '</option>';
                    }
...
function editGrosserrordata() {
    ...
    form.on('submit(editresidualpreview)', function (data) {
        ...
        if (fittingmethod == 'polynomial') {
            preDeletedata = calculateResiduals(fittingmethod, resData, threshold);
        } else {
            preDeletedata = calculateResiduals(fittingmethod, resData, threshold);
        }
        //剔除
...
function editMonitorCurvedata() {
    ...
    form.on('select(editMonitorCurveMethodFilter)', function (data) {
        if (data.value == "polynomial") {
            ...
            $('#editMonitorCurveOrderid').prop("disabled", 'disabled');
        }
    });
    ...
    form.on('submit(editMonitorCurvesubmit)', function (data) {
        ...
        var endTime = Math.round(new Date(result.editMonitorCurveEndTime) / 1000) * 1000;
        var key = result.editMonitorCurve;
        var fittingmethod = result.editMonitorCurveMethod;
        var order = result.editMonitorCurveOrder;
        var fittData = [];
        for (var i in autoOriginalData.Datas) {
            var time = Math.round(new Date(autoOriginalData.Datas[i].Time) / 1000) * 1000;
            if (time > starTime && time < endTime && autoOriginalData.Datas[i].Flag != '200') {
                ...
                preselect.push(Math.round(new Date(autoOriginalData.Datas[i].Time) / 1000) * 1000);
                preselect.push(autoOriginalData.Datas[i][key]);
                fittData.push(preselect);
            }
        }
        var curve_result = fittingMonitorCurvedata(fittingmethod, fittData, order);
        curve_results += curve_result + "\n";
        if (curve_results != "") {
            form.val("editMonitorCurvedataform", {
                "Curveresults": curve_results
            });
        }
        return false;
    });
...
//清除拟合曲线
function clearFitCurve() {
    var chartOption = editChart.getOption();
    var seriesdatas = chartOption.series;
    var newseriesdatas = [];
    for (var i in seriesdatas) {
        if (seriesdatas[i].id != 'fitcurve') {
            newseriesdatas.push(seriesdatas[i]);
        }
    }
    chartOption.series = newseriesdatas;
    editChart.setOption(chartOption, true);
}
//计算标准差
...
//计算残差
function calculateResiduals(type, data, threshold) {
...
//拟合目标监测曲线
function fittingMonitorCurvedata(type, data, order) {
    const xMin = data[0][0];
    ...
    var regressData = myRegression.points;
    for (var i in regressData) {
        regressData[i][0] = regressData[i][0] * (xMax - xMin) + xMin;
    }
    editChart.setOption({
        series: [{
            id: "fitcurve",
    ...
    var ss = "((x- " + (xMin).toString() + ") / " + (xMax - xMin).toString() + ")";
    var expression = myRegression.expression.replaceAll('+ -', '-').replaceAll('x', ss);
    var timelenght = timeTransform(xMin) + "<x<" + timeTransform(xMax);
    expression = expression + " (" + timelenght + ")";

    $('#editFittingFormulaid').val(expression);
    return expression;
...
function PushManage() {
    ...
    //监听提交
    form.on('submit(yuZhiUpdate)', function (data) {
        ...
        if (data.field.type == "裂缝") {
            ...
            if (isNaN(data.field.lfThreshold)) {//是数字的情况
                layer.msg("裂缝阈值请输入数字", { zIndex: layer.zIndex, success: function (layero) { layer.setTop(layero); } });
                return false;
            }
            var threshold = {
            ...
            if (isNaN(data.field.ylThreshold)) {//是数字的情况
                layer.msg("应力阈值请输入数字", { zIndex: layer.zIndex, success: function (layero) { layer.setTop(layero); } });
                return false;
            }
            var threshold = {
                "yl_t": data.field.ylThreshold
            }
            sendDate.Threshold = JSON.stringify(threshold);
            ...
            if (isNaN(data.field.xygnssThreshold)) {//是数字的情况
                layer.msg("GNSS水平阈值请输入数字", { zIndex: layer.zIndex, success: function (layero) { layer.setTop(layero); } });
                return false;
            }
            if (data.field.hgnssThreshold.length == 0) {//是数字的情况
                layer.msg("请输入GNSS垂直阈值", { zIndex: layer.zIndex, success: function (layero) { layer.setTop(layero); } });
                return false;
            }
            if (isNaN(data.field.hgnssThreshold)) {//是数字的情况
                layer.msg("GNSS垂直阈值请输入数字", { zIndex: layer.zIndex, success: function (layero) { layer.setTop(layero); } });
                return false;
            }
            var threshold = {
                "xy_t": data.field.xygnssThreshold,
            ...
            if (isNaN(data.field.qjThresholdx)) {//是数字的情况
                layer.msg("倾角X方向请输入数字", { zIndex: layer.zIndex, success: function (layero) { layer.setTop(layero); } });
                return false;
            }
            if (data.field.qjThresholdy.length == 0) {//是数字的情况
                layer.msg("请输入倾角Y方向", { zIndex: layer.zIndex, success: function (layero) { layer.setTop(layero); } });
                return false;
            }
            if (isNaN(data.field.qjThresholdy)) {//是数字的情况
                layer.msg("倾角Y方向请输入数字", { zIndex: layer.zIndex, success: function (layero) { layer.setTop(layero); } });
                return false;
            }
            if (data.field.qjThresholdz.length == 0) {//是数字的情况
                layer.msg("请输入倾角Y方向", { zIndex: layer.zIndex, success: function (layero) { layer.setTop(layero); } });
                return false;
            }
            if (isNaN(data.field.qjThresholdz)) {//是数字的情况
                layer.msg("倾角Y方向请输入数字", { zIndex: layer.zIndex, success: function (layero) { layer.setTop(layero); } });
                return false;
            }
            var threshold = {
                "x_t": data.field.qjThresholdx,
                "y_t": data.field.qjThresholdy,
                "z_t": data.field.qjThresholdz
            }
            sendDate.Threshold = JSON.stringify(threshold);
        }
        ...
        var loadinglayerindex = layer.load(0, { shade: false, zIndex: layer.zIndex, success: function (loadlayero) { layer.setTop(loadlayero); } });
        $.ajax({
            url: servicesurl + "/api/PatrolEquipment/updateyuZhiInfo", type: "post", data: sendDate,
            success: function (result) {
                layer.close(loadinglayerindex);
                console.log(result);
                var res = JSON.parse(result);
                if (res.code == 1) {//成功
                    layer.msg(data.field.monitorName + "阈值修改成功", { zIndex: layer.zIndex, success: function (layero) { layer.setTop(layero); } });
                } else {
    ...
    //监听提交
    form.on('submit(closePush)', function (data) {
        console.log(data);
        var sendDate = {
        ...
        var loadinglayerindex = layer.load(0, { shade: false, zIndex: layer.zIndex, success: function (loadlayero) { layer.setTop(loadlayero); } });
        $.ajax({
            url: servicesurl + "/api/PatrolEquipment/updateDevicePushInfo", type: "post", data: sendDate,
            success: function (result) {
                layer.close(loadinglayerindex);
                console.log(result);
                var res = JSON.parse(result);
                if (res.code == 1) {//成功
                    layer.msg(data.field.monitorName + "关闭推送成功", { zIndex: layer.zIndex, success: function (layero) { layer.setTop(layero); } });
                    $("#closePush").hide();
                    $("#openPush").show();
    ...
    //监听提交
    form.on('submit(openPush)', function (data) {
        var sendDate = {
            deviceId: data.field.deviceId,
            deviceStatus: "1"//关闭推送
        };
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                var res = JSON.parse(result);
                if (res.code == 1) {//成功
                    layer.msg(data.field.monitorName + "启动推送成功", { zIndex: layer.zIndex, success: function (layero) { layer.setTop(layero); } });
                    $("#closePush").show();
                    $("#openPush").hide();
                    form.val("pushdataform", {
                        deviceStatus: "正在推送"
    ...
    //初值修改
    form.on('submit(InitialPush)', function (data) {
        console.log(data);

        // var data = form.val('example');
        var sendDate = {
            initialValueId: data.field.initialValueId
        };

        if (data.field.type == "裂缝") {
            if (data.field.lfInitialValue.length == 0) {//是数字的情况
                layer.msg("请输入裂缝初值", { zIndex: layer.zIndex, success: function (layero) { layer.setTop(layero); } });
                return false;
            }
            if (isNaN(data.field.lfInitialValue)) {//是数字的情况
                layer.msg("裂缝初值请输入数字", { zIndex: layer.zIndex, success: function (layero) { layer.setTop(layero); } });
                return false;
            }
            var threshold = {
                "len": data.field.lfInitialValue
            }
            sendDate.InitialValue = JSON.stringify(threshold);
        } else if (data.field.type == "GNSS") {
            if (data.field.xygnssInitialValue.length == 0) {//是数字的情况
                layer.msg("请输入GNSS的X初值", { zIndex: layer.zIndex, success: function (layero) { layer.setTop(layero); } });
                return false;
            }
            if (isNaN(data.field.xygnssInitialValue)) {//是数字的情况
                layer.msg("GNSS的X初值请输入数字", { zIndex: layer.zIndex, success: function (layero) { layer.setTop(layero); } });
                return false;
            }

            if (data.field.yygnssInitialValue.length == 0) {//是数字的情况
                layer.msg("请输入GNSS的y初值", { zIndex: layer.zIndex, success: function (layero) { layer.setTop(layero); } });
                return false;
            }
            if (isNaN(data.field.yygnssInitialValue)) {//是数字的情况
                layer.msg("GNSS的y初值请输入数字", { zIndex: layer.zIndex, success: function (layero) { layer.setTop(layero); } });
                return false;
            }

            if (data.field.hgnssInitialValue.length == 0) {//是数字的情况
                layer.msg("请输入GNSS垂直初值", { zIndex: layer.zIndex, success: function (layero) { layer.setTop(layero); } });
                return false;
            }
            if (isNaN(data.field.hgnssInitialValue)) {//是数字的情况
                layer.msg("GNSS垂直初值请输入数字", { zIndex: layer.zIndex, success: function (layero) { layer.setTop(layero); } });
                return false;
            }
            var threshold = {
                "x0": data.field.xygnssInitialValue,
                "y0": data.field.yygnssInitialValue,
                "h0": data.field.hgnssInitialValue
            }
            sendDate.InitialValue = JSON.stringify(threshold);
        } else if (data.field.type == "倾角") {
            //修改初值
            if (data.field.qjInitialValuex.length == 0) {//是数字的情况
                layer.msg("请输入倾角X方向", { zIndex: layer.zIndex, success: function (layero) { layer.setTop(layero); } });
                return false;
            }
            if (isNaN(data.field.qjInitialValuex)) {//是数字的情况
                layer.msg("倾角X方向请输入数字", { zIndex: layer.zIndex, success: function (layero) { layer.setTop(layero); } });
                return false;
            }
            if (data.field.qjInitialValuey.length == 0) {//是数字的情况
                layer.msg("请输入倾角Y方向", { zIndex: layer.zIndex, success: function (layero) { layer.setTop(layero); } });
                return false;
            }
            if (isNaN(data.field.qjInitialValuey)) {//是数字的情况
                layer.msg("倾角Y方向请输入数字", { zIndex: layer.zIndex, success: function (layero) { layer.setTop(layero); } });
                return false;
            }
            if (data.field.qjInitialValuez.length == 0) {//是数字的情况
                layer.msg("请输入倾角Y方向", { zIndex: layer.zIndex, success: function (layero) { layer.setTop(layero); } });
                return false;
            }
            if (isNaN(data.field.qjInitialValuez)) {//是数字的情况
                layer.msg("倾角Y方向请输入数字", { zIndex: layer.zIndex, success: function (layero) { layer.setTop(layero); } });
                return false;
            }
            var threshold = {
                "x0": data.field.qjInitialValuex,
                "y0": data.field.qjInitialValuey,
                "z0": data.field.qjInitialValuez
            }
            sendDate.initialValue = JSON.stringify(threshold);
        }
        ...
        layer.confirm('确认修改初值?', { icon: 3, title: '提示', zIndex: layer.zIndex, success: function (layero) { layer.setTop(layero); } }, function (index) {
            var loadinglayerindex = layer.load(0, { shade: false, zIndex: layer.zIndex, success: function (loadlayero) { layer.setTop(loadlayero); } });
            $.ajax({
                url: servicesurl + "/api/PatrolEquipment/updateChuZhiInfo", type: "post", data: sendDate,
                success: function (result) {
                    layer.close(loadinglayerindex);
                    console.log(result);
                    var res = JSON.parse(result);
                    if (res.code == 1) {//成功
                        layer.msg(data.field.monitorName + "初值修改成功", { zIndex: layer.zIndex, success: function (layero) { layer.setTop(layero); } });
                    } else {
                        layer.msg(res.message, { zIndex: layer.zIndex, success: function (layero) { layer.setTop(layero); } });
                    }
                }, datatype: "json"
    ...
    pushFailuretable = table.render({
        ...
        , toolbar: '#pilianPush'
        , totalRow: false
        , initSort: { field: 'pushTime', type: 'desc' }
        , cols: [[
            { type: 'checkbox' }
            , { field: 'zbh', title: '推送编号', align: "center" }
            , { field: 'pushTime', title: '推送时间', align: "center" }
            , { field: 'value', title: '数据值', align: "center" }
            , { width: 220, align: 'center', toolbar: '#table-toolbar-push' }
            , { field: 'value', title: '', width: 0.1, align: "center" }
        ]]
        , data: []
    });
    table.on('tool(pushdatastatistics)', function (obj) {
        ...
        if (layEvent == "delete") {
            ...
            layer.confirm('确认删除记录?', { icon: 3, title: '提示', zIndex: layer.zIndex, success: function (layero) { layer.setTop(layero); } }, function (index) {
                var loadingminindex = layer.load(0, { shade: 0.3, zIndex: layer.zIndex, success: function (loadlayero) { layer.setTop(loadlayero); } });
                $.ajax({
                    url: servicesurl + "/api/PatrolEquipment/deleteFailalInfo", type: "put", data: { "id": obj.data.id },
                    success: function (result) {
            ...
            //向平台推数据，然后，我们这边删除，失败数据。
            layer.confirm('确认推送数据?', { icon: 3, title: '提示', zIndex: layer.zIndex, success: function (layero) { layer.setTop(layero); } }, function (index) {
                var loadingminindex = layer.load(0, { shade: 0.3, zIndex: layer.zIndex, success: function (loadlayero) { layer.setTop(loadlayero); } });
                $.ajax({
                    url: servicesurl + "/api/PatrolEquipment/UpdatePushFailalInfo", type: "put", data: { "id": obj.data.id },
                    success: function (result) {
                        layer.close(loadingminindex);
                        var res = JSON.parse(result);
                        if (res.code == 1) {//成功
    ...
    //新增按钮
    table.on('toolbar(pushdatastatistics)', function (obj) {
        var checkStatus = table.checkStatus(obj.config.id);
        console.log(obj);
        var checkData = checkStatus.data;
        if (checkData.length == 0) {
            layer.msg("请选中数据", { zIndex: layer.zIndex, success: function (layero) { layer.setTop(layero); } });
            return false;
        }
        if (obj.event == "delete") {//删除
            layer.confirm('确认删除选中的数据?', { icon: 3, title: '提示', zIndex: layer.zIndex, success: function (layero) { layer.setTop(layero); } }, function (index) {
                var id = '';
                ...
                var loadingminindex = layer.load(0, { shade: 0.3, zIndex: layer.zIndex, success: function (loadlayero) { layer.setTop(loadlayero); } });
                $.ajax({
                    url: servicesurl + "/api/PatrolEquipment/deleteFailalInfo", type: "put", data: { "id": id },
                    success: function (result) {
                        layer.close(loadingminindex);
                        var res = JSON.parse(result);
                        if (res.code == 1) {//成功
                            layer.msg("删除成功", { zIndex: layer.zIndex, success: function (layero) { layer.setTop(layero); } });
                            for (var i = shiBaiDataList.length - 1; i >= 0; i--) {
                                for (var j = checkData.length - 1; j >= 0; j--) {
                                    if (shiBaiDataList[i].id == checkData[j].id) {
                                        shiBaiDataList.splice(i, 1);
            ...
            layer.confirm('确认推送选中的数据?', { icon: 3, title: '提示', zIndex: layer.zIndex, success: function (layero) { layer.setTop(layero); } }, function (index) {
                var id = '';
                for (var i in checkData) {
                    id += checkData[i].id + '&';
                }
                ...
                var loadingminindex = layer.load(0, { shade: 0.3, zIndex: layer.zIndex, success: function (loadlayero) { layer.setTop(loadlayero); } });
                $.ajax({
                    url: servicesurl + "/api/PatrolEquipment/UpdatePushFailalInfo", type: "put", data: { "id": id },
                    success: function (result) {
                        layer.close(loadingminindex);
                        var res = JSON.parse(result);
                        if (res.code == 1) {//成功
...
function LoadPushDataPreDateTime(monitor, datetime) {
    ...
    $("#lfInitialValue").hide();
    $("#ylInitialValue").hide();
    $("#qjInitialValuex").hide();
    $("#qjInitialValuey").hide();
    $("#qjInitialValuez").hide();
    $("#xyGNSSInitialValue").hide();
    $("#yyGNSSInitialValue").hide();
    $("#hGNSSInitialValue").hide();
    ...
    if (monitor.type == "裂缝") {
        ...
        $("#qjTypey").show();
        $("#qjTypez").show();
        $("#qjInitialValuex").show();
        $("#qjInitialValuey").show();
        $("#qjInitialValuez").show();
    } else if (monitor.type == "GNSS") {
        $("#xyGNSSType").show();
        $("#hGNSSType").show();
        $("#xyGNSSInitialValue").show();
        $("#yyGNSSInitialValue").show();
        $("#hGNSSInitialValue").show();
    } else if (monitor.type == "深部位移") {
        $("#yuZhiUpdateId").hide();
    }
    ...
    $.ajax({
        url: servicesurl + "/api/Data/GetPushDataPreDateTime", type: "get", data: { "id": monitor.id, "predatetime": datetime, "cookie": document.cookie },
        success: function (data) {
            layer.close(loadingpushindex);
            console.log(monitor);
            console.log(data);
            ...
            if (data != "") {
                ...
                form.val("pushdataform", {
                    ...
                    ThresholdId: fanHuiData.thresholdId,
                    initialValueId: fanHuiData.initialValueId,
                    type: monitor.type,
                    deviceId: fanHuiData.deviceId,
                    deviceStatus: fanHuiData.deviceStatus == "1" ? "正在推送" : "已关闭推送"
                ...
                if (fanHuiData.threshold != null) {
                    if (monitor.type == "裂缝") {
                        ...
                        form.val("pushdataform", {
                            lfThreshold: lfThreshold.len_t
                        })
                        if (fanHuiData.initialValue != null) {//初始值
                            var lfInitialValue = JSON.parse(fanHuiData.initialValue);
                            form.val("pushdataform", {
                                lfInitialValue: lfInitialValue.len,
                            })
                            $("#InitialPush").show();
                        } else {
                            $("#lfType").hide();
                        ...
                        form.val("pushdataform", {
                            qjThresholdx: qjThreshold.x_t,
                            qjThresholdy: qjThreshold.y_t,
                            qjThresholdz: qjThreshold.z_t,
                            initialValueId: fanHuiData.initialValueId
                        })
                        if (fanHuiData.initialValue != null) {
                            var qjInitialValue = JSON.parse(fanHuiData.initialValue);
                            console.log(qjInitialValue);
                            form.val("pushdataform", {
                                qjInitialValuex: qjInitialValue.x0,
                                qjInitialValuey: qjInitialValue.y0,
                                qjInitialValuez: qjInitialValue.z0,
                            })
                            $("#InitialPush").show();
                        } else {
                            $("#qjInitialValuex").hide();
                        ...
                        console.log(qjThreshold);
                        form.val("pushdataform", {
                            xygnssThreshold: qjThreshold.xy_t,
                            hgnssThreshold: qjThreshold.h_t,
                        })
                        if (fanHuiData.initialValue != null) {//初始值GNSS
                            var qjinitialValue = JSON.parse(fanHuiData.initialValue);
                            console.log(qjinitialValue);
                            form.val("pushdataform", {
                                xygnssInitialValue: qjinitialValue.x0,

                                yygnssInitialValue: qjinitialValue.y0,
                                hgnssInitialValue: qjinitialValue.h0,
                            })
                            $("#InitialPush").show();
                        } else {
                            $("#xyGNSSInitialValue").hide();
